#ifndef __ESP_TIME_ALARM_H
#define __ESP_TIME_ALARM_H
...
typedef std::function<void(void*)> OnTick_with_arg_t;
#else
typedef void (*OnTick_t)(void);             // alarm callback function typedef
typedef void (*OnTick_with_arg_t)(void*);   // alarm callback function typedef
#endif
...
public:
...
#if defined(ARDUINO_ARCH_ESP32) || defined(ARDUINO_ARCH_ESP8266)
  AlarmID_t alarmRepeat(const int H, const int M, const int S, OnTick_with_arg_t onTickHandler, void *arg) {
    alarmRepeat(AlarmHMS(H,M,S), std::bind(onTickHandler, arg));
  }

  AlarmID_t alarmRepeat(const timeDayOfWeek_t DOW, const int H, const int M, const int S, OnTick_with_arg_t onTickHandler, void *arg) {
    time_t value = (DOW-1) * SECS_PER_DAY + AlarmHMS(H,M,S);
    if (value <= 0) return dtINVALID_ALARM_ID;
    return create(value, std::bind(onTickHandler, arg), false, dtWeeklyAlarm);
  }
#endif
  ...
  void freeID(AlarmID_t ID);                        // free the id to allow its reuse
